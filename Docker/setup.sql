...
USE cesimanger;
GO
CREATE TABLE Users (ID int IDENTITY(1,1) PRIMARY KEY, user_name nvarchar(max), password binary(20), name nvarchar(max), surname nvarchar(max), telephone int, email nvarchar(max), address nvarchar(max), zip nvarchar(max), city nvarchar(max), country nvarchar(max), status nvarchar(max), created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP);
GO
ALTER TABLE Users
